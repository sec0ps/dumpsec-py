                ...
                for item in content:
                    if isinstance(item, dict):
                        for k, v in item.items():
                            elements.append(Paragraph(f"• {k}: {v}", styles["Content"]))
                        elements.append(Spacer(1, 0.1 * inch))  # Spacing between items
                    else:
                ...
                for key, value in content.items():
                    ...
                    if isinstance(value, list):
                        if all(isinstance(v, dict) for v in value):
                            elements.append(Paragraph(f"{key}:", styles["Content"]))
                            for v in value:
                                for k2, v2 in v.items():
                                    elements.append(Paragraph(f"• {k2}: {v2}", styles["Content"]))
                                elements.append(Spacer(1, 0.1 * inch))
                        else:
                            elements.append(Paragraph(f"{key}:", styles["Content"]))
                            for v in value:
                                elements.append(Paragraph(f"• {v}", styles["Content"]))
                    elif isinstance(value, dict):
                        elements.append(Paragraph(f"{key}:", styles["Content"]))
    ...
    try:
        with open(path, "w", encoding="utf-8") as f:
            ...
            <style>
                ...
                table { border-collapse: collapse; margin-left: 40px; margin-bottom: 10px; }
            ...
            for section, content in data.items():
                ...
                if isinstance(content, dict):
                    for key, value in content.items():
                        ...
                        # Handle tuple format like ([entries], [risks])
                        if isinstance(value, tuple) and len(value) == 2 and isinstance(value[0], list):
                            value, extra_risks = value
                            content.setdefault("_risks", []).extend(extra_risks)
